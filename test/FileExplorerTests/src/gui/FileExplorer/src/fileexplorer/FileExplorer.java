...
import clg.project.summer.others.FileExplorer.src.fileexplorer.*;
import clg.project.summer.others.FileExplorer.src.amaras.runtime.*;
import clg.project.summer.others.FileExplorer.src.amaras.icon.*;
...
public class FileExplorer extends JFrame implements WindowListener {
   ...
   private void helperConstructor() {
     ...
     if (isMac == false) {
      ...
      // Should be placed in this location (sample): "E:\Projects\Java\IntelliJ IDEA\out\production\Swing\strings.properties"
      ...
      //Paste Thread setup
      if (pasteque == null) {
         pasteque    = new PasteQue(transferlist);
      }
      
      if (pasteworker == null) {
         pasteworker = new PasteWorker(pasteque);
      ...
      showToolbar = prefsView.getBoolean(View.SHOW_TOOLBAR,
                    View.SHOW_TOOLBAR_DEFAULT);
...
//      setIconImage(logo.getImage());
      ...
      location = prefsLoc.get(Location.START_LOCATION,
...
//      transferList      = new transferListClass( resbundle.getString("transferList") );
      ...
      sortListAction    = new sortListActionClass[DirectoryView.SORT_OPTIONS];
   ...
   public void addMenus() {
      ...
      hiddenMenuItem.setSelected(prefsView.getBoolean(
                  View.SHOW_HIDDEN, View.SHOW_HIDDEN_DEFAULT));
      ...
      m_sort = new JMenuItem[DirectoryView.SORT_OPTIONS];
      int defMode = prefsView.getInt(View.SORT_MODE, View.SORT_MODE_DEFAULT);
      for (int i=0; i<m_sort.length; i++) {
         m_sort[i] = new JRadioButtonMenuItem(DirectoryView.text[i]);
